...
data=data[3:80,c(2:dim(data)[2])]

data_sum_change <-function(data)
{    
data1=matrix(0,dim(data)[1],31)
for(i in 1:dim(data)[1])
    {
    if(i%%4==1) data1[i,]=as.matrix(data[i,])
    if(i%%4!=1) data1[i,]=as.matrix(data[i,]-data[(i-1),])   
}
return(data1)
}
data=data_sum_change(data)
...
etsxgbreg <- readRDS("Q_etsxgbreg.rds")
etsxgbcls <- readRDS("Q_etsxgbcls.rds")
etslgbreg <- readRDS("Q_etslgbreg.rds")
etslgbcls <- readRDS("Q_etslgbcls.rds")

thetafxgbreg <- readRDS("Q_thetafxgbreg.rds")
thetafxgbcls <- readRDS("Q_thetafxgbcls.rds")
thetaflgbreg <- readRDS("Q_thetaflgbreg.rds")
thetaflgbcls <- readRDS("Q_thetaflgbcls.rds")
...
write.csv(alltest,'china_industry_M4.csv')
